...
float[] TrajX = new float[10];
float[] TrajY = new float[10];
float[] TrajZ = new float[10];
int TrajPt=0;
PGraphics p1,p3,p2;
...
void IK(){
  ...
  float Z = PtZ.getValue();
// if (ProcessIK.getState()==true){
  float L = sqrt(Y*Y+X*X);
  float dia = sqrt(Z*Z+L*L);

  PointX = PI/2-(atan2(L, Z)+acos((T*T-F*F-dia*dia)/(-2*F*dia)));
  PointY = -PI+acos((dia*dia-T*T-F*F)/(-2*F*T));
  PointZ = atan2(Y, X);
// }
// else
// {
