...
float[] TrajX = new float[50];
float[] TrajY = new float[50];
float[] TrajZ = new float[50];
...
void DrawTraj(PGraphics pg){
   ...
   pg.stroke(100);
   int j;
   if(TrajPt==0){j=TrajX.length-1;}
   else{j=TrajPt-1;}
   float k=TrajX.length+1;
for (int i=TrajX.length; i > 0; i--) {
   pg.pushMatrix();
   
  
     pg.translate(TrajX[j], TrajY[j], TrajZ[j]);
     j--; if (j<0)j=TrajX.length-1;
     if(k>0)k--;
     pg.fill (0, 25);
     pg.stroke(100);
     pg.sphere (2*-k/TrajX.length);
    // print ("pop");
   pg.popMatrix();
 //  print((k/5+1)+" ");
   }
...
void DrawAxes(PGraphics pg,int n,int len,boolean grid){
 ...
 //    pg.rotateZ(PI/2);}
   ...
   float intensity = GridIntensity.getValue();
 
   if(ShowGridX.getState()==true) {//pg.stroke(30,30,30);
   for(int x=-len/20; x<=len/20 ;x++)
    {
      
      int  j= x%5; if (j==0){pg.stroke(intensity*0.8,0,0);}else{pg.stroke(intensity*0.6,0,0);}
      j= x%10; if (j==0){pg.stroke(intensity*1.6,0,0);}
   ...
   if(ShowGridY.getState()==true) {//pg.stroke(30,30,30);
    ...
    {
      int  i= y%5; if (i==0){pg.stroke(0,intensity*0.4,0);}else{pg.stroke(0,intensity*0.4,0);}
      i= y%10; if (i==0){pg.stroke(0,intensity*0.8,0);}
   ...
   if(ShowGridZ.getState()==true) {//pg.stroke(30,30,30);
    ...
    {
      int  i= z%5; if (i==0){pg.stroke(0,0,intensity*0.4);}else{pg.stroke(0,0,intensity*0.40);}
      i= z%10; if (i==0){pg.stroke(0,0,intensity*0.80);}
...
//   SliderAxe4.setValue(*180/PI);
//   SliderAxe5.setValue(*180/PI);
//   SliderAxe6.setValue(*180/PI);
   ...
   RotAxe4 = SliderAxe4.getValue()*PI/180;   // en attente des calculs
