...
void mouseDragged() {
 if (mouseX>500){
  ...
  {
     //TranslateX=TranslateX+(mouseX - pmouseX) * abs(TransZ)/150;  
     //TranslateY=TranslateY+(mouseY - pmouseY) * abs(TransZ)/150;
    if (abs(TransZ)>10) {
      TranslateX=TranslateX+(mouseX - pmouseX) * abs(TransZ)/150;  
      TranslateY=TranslateY+(mouseY - pmouseY) * abs(TransZ)/150;
    } else {
      TranslateX=TranslateX+(mouseX - pmouseX)*10 ;  
      TranslateY=TranslateY+(mouseY - pmouseY)*10 ;
    }
  } else {
      rotY -= (mouseX - pmouseX) * 0.01;
      rotX -= (mouseY - pmouseY) * 0.01;
    //  println(rotX*180/PI+";"+rotY*180/PI+";"+TransZ);
  }
 }
}
...
void serialEvent2(Serial myPort) {
  ...
  try {
    ...
    if (val!=null)
    {
      ...
      if (val.indexOf('\r')>0) { 
        val.substring(0, val.indexOf('\r'));      
        val=trim(val);
      }
      if (val!=null  && val.indexOf(">")<=1 && val.length()<=2) 
      {
        if ( ShowAll.getState() == false ) { 
          output(1, "Ok>"); 
          print (">");
        }
...
String decode( String s) {
  ...
  try {
    ...
    //println ("#"+s2+"#");  
    String oZ=""; 
    if (s2.indexOf("Offz=")>=0) { 
      oZ = s2.substring(s2.indexOf("Offz=")   +5, s2.indexOf("delta x=")-1);
    }  //println("#"+oZ+"#");
    print (" offZ["+oZ+"]");
    String mX=""; 
    if (s2.indexOf("delta x=")>=0) { 
      mX = s2.substring(s2.indexOf("delta x=")+8, s2.indexOf("y=")-1);
    }        //println("#"+mX+"#");
    String mY=""; 
    if (s2.indexOf("y=")>=0) { 
      mY = s2.substring(s2.indexOf("y=")      +2, s2.indexOf(" z="));
    }         //println("#"+mY+"#");
    String mZ=""; 
    if (s2.indexOf(" z=")>=0) { 
      mZ = s2.substring(s2.indexOf(" z=")     +3);
    }                           //println("#"+mZ+"#");
